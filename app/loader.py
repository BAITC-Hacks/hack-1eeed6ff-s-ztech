...
def _money(frame, name):
    ...
    if len(bad):
        raise DataValidationError(f'{name}.sum_kzt: precision exceeds one tiyin at row {int(bad[0])}')
    if (rounded >= 2**63).any():
...
def load_dataset(data_dir: Path, profile: str = 'generic') -> Dataset:
    ...
    for name, columns in COLUMNS.items():
        ...
        try:
            ...
            raise DataValidationError(f'{path.name}: invalid parquet ({type(exc).__name__})') from exc
    ...
    for src, dst, amount, count in edges[['src', 'dst', 'sum_tiyin', 'n_tx']].itertuples(index=False, name=None):
        ...
        if delta > 1:
            raise DataValidationError(f'edges: aggregate sum mismatch for {src}->{dst}: {delta} tiyin')
        tolerance_pairs += int(delta != 0)
        canonical.append(total)
    edges['sum_tiyin'] = pd.Series(canonical, dtype=object)
    audit.update(rows={k: len(v) for k, v in tables.items()}, source_hashes=hashes,
                 ...
                 seeds=int(nodes.is_seed.sum()), profile=profile)
    if profile == 'official':
        manifest = json.loads((Path(__file__).resolve().parents[1] / 'docs/hackalem/sources/manifest.json').read_text())
        expected = {Path(x['path']).name: x['sha256'] for x in manifest['files'] if x['path'].endswith('.parquet')}
        if hashes != expected:
            raise DataValidationError('official profile: source hashes do not match organizer files')
        if tuple(len(tables[k]) for k in COLUMNS) != (2248, 3119, 4840):
            raise DataValidationError('official profile: wrong row counts')
    return Dataset(nodes.sort_values('gid').reset_index(drop=True),
