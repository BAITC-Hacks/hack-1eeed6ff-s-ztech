...
G3 пройден на cd94bfb; A реализует этот уже предусмотренный ТЗ endpoint после 15:13 Астана. Существующие GET-поля и CSV не меняются. Это договор для B до реализации UI. Включать действие «Структурная симуляция» только при `meta.features.removal=true` и после реальной проверки; отсутствие UI не мешает основному сценарию.
