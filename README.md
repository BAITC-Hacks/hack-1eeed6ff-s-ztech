...
![Настоящий интерфейс Neverlose: очередь проверки слева, направленный граф в центре, карточка с основаниями справа](docs/hackalem/evidence/agent-ui-a/final-floating-button-dark.png)
...
Панель помогает разобрать роль и альтернативу, проверить исходные переводы и выбрать следующий запрос данных. Поддерживаются вопросы по текущему графу; произвольные вычисления, поиск общего получателя нескольких счетов и трассировка маршрутов пока не реализованы. Обзор содержит пять лидеров, список соседей ограничен 20 связями.
...
Приложение строит граф, роли, кластеры и три CSV; поиск, исходные переводы, альтернативные гипотезы, обе темы и AI-панель проверены в браузере. Проходят 133 backend-теста, 31 frontend-тест, 50 браузерных contract-проверок и 3 сквозных сценария на настоящих данных с запрещённой внешней сетью; отдельно подтверждены настоящие вопросы к OpenAI. Структурная симуляция пока доступна через API, временные паттерны не реализованы; основной сценарий работает без ключа. Точный SHA: `git rev-parse HEAD`; [проверки и ограничения](docs/validation.md), [текущее состояние](docs/hackalem/STATUS.md), [порядок подачи](docs/SUBMISSION.md).
