...
![Настоящий интерфейс Neverlose: очередь проверки слева, направленный граф в центре, карточка с основаниями справа](docs/hackalem/evidence/b5-a/real-card-dark-1440.png)

Скриншот реального расчёта, интерфейс B5. Слева — кого проверить; в центре — связи; справа — почему выбран узел. Суммы ведут к исходным переводам. Поиск принимает любой полный gid, включая изолированный seed. Полезный результат работы — объяснимый приоритет, исходные основания и следующий запрос данных, а не только изображение графа.
...
Откройте **http://127.0.0.1:8000**. В окне: **2 248 узлов, 3 119 связей, 4 840 переводов**. В терминале — `status=ready`, `run_id` и время пересчёта; в `results/` — три CSV. Остановка — **Ctrl+C**. Если порт занят: `python run.py --port 8001`, затем откройте порт 8001.
...
Версия вычислительного кода `1.0.1`; изменения этой версии исправляют большие суммы и пользовательские настройки, три CSV официального набора остаются прежними. Подробные формулы: [02_DATA_ALGORITHMS.md](docs/hackalem/02_DATA_ALGORITHMS.md). Действующие параметры: [config/rules.json](config/rules.json), версия `v1`.
...
Основному сценарию этот раздел не нужен. После `cp .env.example .env` укажите свой ключ в `OPENAI_API_KEY` через локальный редактор. `.env` исключён из Git; не помещайте ключ в командную строку, browser/VITE переменные или коммиты. По умолчанию модель `gpt-5.4-mini-2026-03-17`; `OPENAI_MODEL` задаёт другую доступную вашему аккаунту Responses-модель, но другие модели не проходили нашу приёмку. Переменные окружения имеют приоритет над `.env`.
...
- E1 доступен через API; UI-панель ещё не включена. E2 не реализован. F13 — настоящий ограниченный LLM-цикл через API/CLI, без интерфейса чата; по умолчанию выключен. Детерминированные объяснения ядра работают независимо от него.
...
**Состояние реализации:** ядро, три CSV, API и интерфейс Алишера B5 объединены. Поиск, направленный граф, основания ролей, альтернативы, исходные переводы, кластеры и скачивание CSV проверены на настоящих данных. 130 backend-тестов (включая 16 E1 и 25 F13), 19 frontend-тестов, 34 браузерных contract-теста и 3 сквозных сценария проходят. Обе темы и возврат к обзору проверены на трёх размерах экрана; новый B5 также прошёл сценарии с запрещённой внешней сетью. Исторические проверки новых clone на обоих ноутбуках и отдельный новый clone после F13 описаны в журнале; новый clone после B5 не заявляется. API структурной симуляции готов; его UI ещё не включён. Необязательный OpenAI-аналитик проверен через API/CLI на реальной модели; его UI пока отсутствует. Подача на платформе не подтверждена. [Актуальный статус](docs/hackalem/STATUS.md), [проверки](docs/validation.md), [журнал A](docs/progress/A.md), [журнал B](docs/progress/B.md).

Последняя опубликованная приёмка B5: [8b8ef87](https://github.com/BAITC-Hacks/hack-1eeed6ff-s-ztech/commit/8b8ef8775b6fbd071bb357b055d19efe08604341), код интерфейса `50bf33a`. Это проверенная база, а не объявление окончательной подачи. Исторический MVP `ae48cab` предшествует этим изменениям. [Evidence B5](docs/hackalem/evidence/b5-a/report.json) содержит команды, время, counts и hashes; дальнейшие проверки добавляются в [validation](docs/validation.md). Версия вашего checkout: `git rev-parse HEAD`.

Перед отправкой выбранный полный SHA и время фиксируются в [SUBMISSION](docs/SUBMISSION.md). Архитектурная схема: [docs/architecture.md](docs/architecture.md), сценарий до пяти минут: [docs/demo.md](docs/demo.md). Устное выступление человека ещё не измерено.

Дедлайн **23 сентября 2026, 18:00 Астана (UTC+5)**. История и отчёты отражают фактическую работу. Подача на платформе не подтверждена: push в GitHub не равен отправке решения. [Сведения и действия для подачи](docs/SUBMISSION.md). После дедлайна изменения не считаются оцениваемой версией.
