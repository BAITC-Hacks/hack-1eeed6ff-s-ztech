...
Это самостоятельный интерфейс Neverlose, без логотипа банка и без заявления об аффилиации.
...
не подключена. E2 отсутствует. Агент реализован A через API/CLI и отдельный web/src/agent; фактические инструменты показываются после ответа сервера. AI-кнопка и легенда web/src/analysis подключаются в App без изменения визуальных компонентов B. Симуляция инструментов не используется.
