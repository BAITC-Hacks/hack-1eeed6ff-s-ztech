...
В backend появился необязательный агент API/CLI, но эта UI-версия его не вызывает.
...
не подключена. E2 отсутствует. Агент реализован A через необязательный API/CLI;
агентский UI не подключён, проверенный запуск B5 имеет features.agent=false.
