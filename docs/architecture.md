...
Если выбор узла скрывает сфокусированную очередь/таблицу на узком экране, фокус
после commit переходит в заголовок видимой карточки. Если поле поиска осталось
видимым, его фокус сохраняется. Кнопки входа/выхода имеют доступное описание с
точной суммой; имя боковой панели различает карточку узла и кластера. На узких
экранах разворот графа подписан текстом. Verifier также сверяет выбранный score с
кандидатом/caps и supporting_transfers с точными исходными строками/суммами/порядком.
...
## Общие прямые получатели

Аддитивный GET /api/v1/analysis/common-recipients и восьмой инструмент агента используют одну чистую функцию app/common_recipients.py. Полный проход по raw transfers, точные тиыны, сохранённые повторы, coverage до limit и source_refs. Offline-панель в web/src/agent позволяет собрать 2–5 gid, увидеть результат, перейти в карточку и подготовить явный вопрос без автоматического обращения к OpenAI. Группа сохраняется между закрытиями dialog; ответы защищены LatestRequest и общим run_id.

Агент по умолчанию gpt-6-astra (reasoning low). Для успешного common-tool сервер фиксирует полный распознанный набор и порог=N, обязательно показывает результаты и ограничения; явно перечисленная группа имеет приоритет перед открытой карточкой. Это не гарантирует понимание любого вопроса моделью. [Контракт](../contracts/common-recipients-v1.md),[проверки](hackalem/evidence/common-a/report.json). Основное ядро/CSV/run_id не изменены.
